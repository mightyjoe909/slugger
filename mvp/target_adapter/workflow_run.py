...
def _validate_candidate_policy() -> None:
    ...
    while index < len(records):
        ...
        if len(record) < 4 or record[2] != " ":
            raise RuntimeError("git returned malformed candidate status")
        changed.append(record[3:])
        # With -z, rename/copy records contain the destination in this record and
        # the source as the next NUL-delimited field (with no status prefix).
...
def _publish(request: ExecutionRequest, branch: str, token: str) -> ManagedPullRequest:
    ...
    try:
        ...
        decision = reconcile(
            request,
            _prs(token),
            branch_exists=_branch_exists(branch, token),
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    token = os.environ[
        "GITHUB_TOKEN" if args.verify_only else "SLUGGER_PUBLICATION_TOKEN"
    ]
    try:
        decision = reconcile(
            request,
